'''
Generates myofiber directions on a truncated biventricular geometry using 
...
Input:
    ...
    2. Mesh surfaces: top.vtp, epi.vtp, endo_lv.vtp, endo_rv.vtp
...
def loadLaplaceSoln(fileName):
    '''
    Load a solution to a Laplace-Dirichlet problem from a .vtu file
    '''
...
#----------------------------------------------------------------------
def getFiberDirections(numCells, Phi_EP, Phi_LV, Phi_RV, \
...
if __name__ == '__main__':
    ...
    # Name of the mesh vtu file on which to compute the fiber directions
    fname = fd_name + "VOLUME.vtu"
    
    # Whether to solve the Laplace equation or not
    run_flag = True

    # Generate epi_apex and epi_mid from epi surface
    top_name = fd_name + "mesh-surfaces/BASE.vtp"
    ...
    for j in range(epi_apex_cells_surf.shape[0]):
        for k in range(epi_apex_cells_surf.shape[1]):
            ...
            epi_apex_cells[j, k] = temp_index[0]
    ...
    for j in range(epi_mid_cells_surf.shape[0]):
        for k in range(epi_mid_cells_surf.shape[1]):
            ...
            temp_index = temp_index[0]
            epi_mid_cells[j, k] = temp_index[0]
    ...
    # Solve 4 Laplace Equations simultaneously for Phi_EPI, Phi_LV, Phi_RV, Phi_AB
    if run_flag == True:
        in_name = "svFSI_BiV.xml"
        out_name = os.path.join(fd_name, "heat/fibers", in_name)
        ...
        for item in range(len(svRead)):
            ...
            if "Face file path: ./top.vtp" in svRead[item]:
                svRead[item] = "      Face file path: " + fd_name + "mesh-surfaces/BASE.vtp" + "\n"
            ...
            if "Face file path: ./endo_lv.vtp" in svRead[item]:
                svRead[item] = "      Face file path: " + fd_name + "mesh-surfaces/LV.vtp" + "\n"
            if "Face file path: ./endo_rv.vtp" in svRead[item]:
                svRead[item] = "      Face file path: " + fd_name + "mesh-surfaces/RV.vtp" + "\n"
            if "Save results in folder:" in svRead[item]:
                svRead[item] = "Save results in folder: " + fd_name + "heat/fibers" + "\n"

        with open(out_name, 'w') as svFileNew:
            svFileNew.writelines(svRead)

        print("Running svFSIPLUS solver")
        os.system(exec_svfsi + out_name)

    # Calculate the polar coordinate of each point
    result_file = fd_name + 'heat/fibers/result_020_cpp.vtu'

    # copy heat results to a folder 
    heat_results_dir = os.path.join(Path(os.getcwd()).parents[2], 'BLNM', 'heat_results')
    
    geom_folder = os.path.basename(os.path.normpath(fd_name))
    
    os.makedirs(heat_results_dir, exist_ok=True)
    shutil.copy2(result_file, os.path.join(heat_results_dir, geom_folder + '_results_020_cpp.vtu'))
    
    result_mesh, Phi_EPI, Phi_LV, Phi_RV, Phi_AB, \
        gPhi_EPI, gPhi_LV, gPhi_RV, gPhi_AB = loadLaplaceSoln(result_file)

    F, S, T = getFiberDirections(NoC, Phi_EPI, Phi_LV, Phi_RV,
    ...
    # fname1 = fd_name + "fibersLong.vtu"
    fname1 = fd_name + "heat/fibers/fibers.vtu"
    print("   Writing to vtu file   --->   %s" % (fname1))
    output_mesh.cell_data.set_array(F, 'FIB_DIR')
    output_mesh.save(fname1)

    # fname1 = fd_name + "fibersSheet.vtu"
    fname1 = fd_name + "heat/fibers/fibersx.vtu"
    print("   Writing to vtu file   --->   %s" % (fname1))
    output_mesh.cell_data.remove('FIB_DIR')
    output_mesh.cell_data.set_array(T, 'FIB_DIR')
    output_mesh.save(fname1)

    # fname1 = fd_name + "fibersNormal.vtu"
    fname1 = fd_name + "heat/fibers/normals.vtu"
    print("   Writing to vtu file   --->   %s" % (fname1))
    output_mesh.cell_data.remove('FIB_DIR')
